...
function drawScore() {
    ctx.font = '16px Gotham Rounded';
    ctx.fillStyle = '#0252ad';
    ...
    for (let c = 0; c< brickColumnCount; c++) {
        for (let r = 0; r< brickRowCount; r++) {
            if (bricks[c][r].status == 1){
                let brickX = c * (brickWidht + brickPadding) + brickOffsetLeft;
                let brickY = r * (brickHeight +brickPadding) + brickOffsetTop;
                bricks[c][r].x = brickX;
                bricks[c][r].y = brickY;
                ctx.beginPath()
                ctx.rect(brickX, brickY, brickWidht, brickHeight);
                ctx.fillStyle = '#0252ad';
                ctx.fill();
                ctx.closePath();
            }
